...
class ControlPanel extends Model {
     ...
     * @var string
     */
    protected $table = 'fr_control_panels';
    ...
    public function unacceptUser( User $user )
    {
        return $this->getUserPivotQuery($user)->update(array(
            'fr_user_control_panel.accepted' => false
        ));
    }
    ...
    {
        return $this->getUserPivotQuery($user)->update(array(
            'fr_user_control_panel.accepted' => true
        ));
    }
    ...
    {
        return $this->users()->where('fr_user_control_panel.user_id', $user->id);
    }
    ...
    public function getAcceptedUsers( User $except = null )
    {
        ...
        if($except)
        {
            $query->where('fr_user_control_panel.user_id', '!=', $except->id);
        }

        return $query->where('fr_user_control_panel.accepted', true)->get();
    }
    ...
    {
        return $this->belongsToMany( User::getClass(), 'fr_user_control_panel' )->withPivot('accepted');
    }
