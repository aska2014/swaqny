...
abstract class User extends Model implements UserInterface, RemindableInterface {
    ...
    public function setInfo( UserInfo $userInfo )
    {
        ...
        return $this->info()->save($userInfo);
    }
    ...
    {
        $infoAttributes = $this->getInfoAttributes();

        return isset($infoAttributes[ $key ]) ? $infoAttributes[ $key ] : '';
    }

    /**
     * Load user information if not already loaded and put these attributes in the information attributes
     *
     * @return array
     */
    public function getInfoAttributes()
    {
        if(empty($this->infoAttributes)) {

            $this->infoAttributes = $this->info()->first()->getAttributes();
        }
    ...
    {
        return $this->hasOne(UserInfo::getClass());
    }
